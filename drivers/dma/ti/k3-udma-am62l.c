...
static int am62l_udma_request(struct dma *dma)
{
	...
	if (uc->config.dir == DMA_MEM_TO_DEV) {
		...
		uc->desc_rx = dma_alloc_coherent(ucc->hdesc_size * UDMA_RX_DESC_NUM, &dummy);
...
static int am62l_udma_get_mmrs(struct udevice *dev, struct udma_dev *ud)
{
	int i;
	...
	for (i = 1; i < AM62L_MMR_LAST; i++) {
		...
		ud->mmrs[i] = dev_read_addr_name_ptr(dev, am62l_mmr_names[i]);
		if (IS_ERR(ud->mmrs[i]))
...
static int am62l_udma_probe(struct udevice *dev)
{
	...
	uc_priv->supported = DMA_SUPPORTS_MEM_TO_DEV | DMA_SUPPORTS_DEV_TO_MEM;
...
static int am62l_udma_remove(struct udevice *dev)
{
	struct udma_dev *ud = dev_get_priv(dev);
...
static const struct udevice_id am62l_udma_ids[] = {
	{
		.compatible = "ti,am62l-dmss-bcdma",
		.data = (ulong)&am62l_bcdma_data,
	},
	{
		.compatible = "ti,am62l-dmss-pktdma",
		.data = (ulong)&am62l_pktdma_data,
	},
	{/* Sentinel */},
};
...
U_BOOT_DRIVER(ti_am62l_udma) = {
	.name	= "ti-am62l-udma",
	.id	= UCLASS_DMA,
	.of_match = am62l_udma_ids,
	.ops	= &am62l_udma_ops,
