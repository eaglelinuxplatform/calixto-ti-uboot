 ...
 * board.c
 *
 * Board functions for Calixto AM335x BorderPAD
 *
...
static const struct ddr_data ddr3_calixto_borderpad_data = {
...
static const struct cmd_control ddr3_calixto_borderpad_cmd_ctrl_data = {
...
const struct dpll_params dpll_ddr_calixto_borderpad = {
		303, OSC-1, 1, -1, -1, -1, -1};

void am33xx_spl_board_init(void)
{
 #if defined(CONFIG_PMIC_TPS65910)
       ...
       /*
        * For all MPU frequencies we support we use a CORE voltage
	* of 1.1375V.  For MPU voltage we need to switch based on
...
const struct dpll_params *get_dpll_ddr_params(void)
{
	...
	return &dpll_ddr_calixto_borderpad;
}
...
void sdram_init(void)
{
	...
	config_ddr(303, CALIXTO_DDR3_IOCTRL_VALUE, &ddr3_calixto_borderpad_data,
		   	&ddr3_calixto_borderpad_cmd_ctrl_data, &ddr3_calixto512_emif_reg_data, 0);
#elif defined(CONFIG_256DDR3)
	config_ddr(303, CALIXTO_DDR3_IOCTRL_VALUE, &ddr3_calixto_borderpad_data,
			&ddr3_calixto_borderpad_cmd_ctrl_data, &ddr3_calixto256_emif_reg_data, 0);
#endif
}
#endif
...
int board_late_init(void)
{
	...
	setenv("board_name", "AM335BP");
	setenv("board_rev", "3.0");
#endif
